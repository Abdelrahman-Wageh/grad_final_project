...
class LocalAIService {
  ...
  /// Generate response from text (LLM only) - renamed to avoid conflict
  ...
  Future<String?> textToSpeech(String text) async {
    final result = await synthesizeSpeech(text: text);
    
    // Save audio to temp file
    final tempDir = Directory.systemTemp;
    final audioFile = File('${tempDir.path}/tts_${DateTime.now().millisecondsSinceEpoch}.wav');
    await audioFile.writeAsBytes(result.audioData);
