@echo off
REM ============================================================================
REM Enhanced Edge Test Script - Whispering Woods Development Tools
...
if not exist "%LOG_DIR%" (
    ...
    if not exist "%LOG_DIR%" (
        ...
        set "LOG_FILE=%LOG_DIR%\edge_test_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%%time:~6,2%.log"
...
if defined LOG_FILE (
    call :log "==========================================" >nul 2>&1
    call :log "Enhanced Edge Test Started" >nul 2>&1
...
REM ============================================================================
REM Step 1: Check Edge (usually pre-installed with Windows)
REM ============================================================================
echo [Step 1/10] Checking Edge installation...
if defined LOG_FILE call :log "Checking Edge..." >nul 2>&1
echo.

where msedge >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    echo   ✓ Edge found in PATH
    if defined LOG_FILE call :log "Edge found" >nul 2>&1
    goto :check_python
)

if exist "%ProgramFiles(x86)%\Microsoft\Edge\Application\msedge.exe" (
    set "EDGE_PATH=%ProgramFiles(x86)%\Microsoft\Edge\Application\msedge.exe"
    echo   ✓ Edge found in Program Files (x86)
    if defined LOG_FILE call :log "Edge found" >nul 2>&1
    goto :check_python
)
...
if exist "%ProgramFiles%\Microsoft\Edge\Application\msedge.exe" (
    ...
    echo   ✓ Edge found in Program Files
    if defined LOG_FILE call :log "Edge found" >nul 2>&1
    goto :check_python
)

REM Edge not found (rare, but possible)
echo   ⚠ Edge not found - may need Windows update
if defined LOG_FILE call :log "Edge not found" >nul 2>&1
set "EDGE_MISSING=1"
...
if not exist "%BACKEND_DIR%" (
    ...
    echo   2. The script should be in: devtools\edge_test.bat
...
REM ============================================================================
echo.
echo [Step 8/10] Opening Edge browser...
if defined LOG_FILE call :log "Opening Edge..." >nul 2>&1
echo.

if defined EDGE_MISSING (
    echo   ⚠ Edge not available - Skipping browser open
...
if defined EDGE_PATH (
    start "" "!EDGE_PATH!" "%FRONTEND_URL%"
    echo   ✓ Edge opened with %FRONTEND_URL%
) else (
    start "" msedge "%FRONTEND_URL%"
    echo   ✓ Edge opened with %FRONTEND_URL%
)

if defined LOG_FILE call :log "Edge opened successfully" >nul 2>&1
...
if not defined EDGE_MISSING (
    echo   ✓ Browser:        Edge opened
) else (
    echo   ⚠ Browser:        Edge not available (servers running)
)
echo.
...
REM Change to frontend directory first (handle spaces in path)
cd /d "%FRONTEND_DIR%"
if %ERRORLEVEL% NEQ 0 (
    echo   ✗ ERROR: Cannot change to frontend directory: %FRONTEND_DIR%
    echo   Check if the path contains special characters
    exit /b 1
)
...
start "Frontend Server - Whispering Woods" cmd /k "cd /d \"%FRONTEND_DIR%\" && echo ============================================================ && echo Frontend Server - Whispering Woods && echo Running on port %FRONTEND_PORT% && echo Press Ctrl+C to stop && echo ============================================================ && npm start"
